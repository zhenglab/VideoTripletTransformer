...
class BaseSTV2Model(BaseModel):
    ...
    def modify_commandline_options(parser, is_train=True):
        parser.set_defaults(norm='instance', netG='vth', dataset_mode='hytsample')
        if is_train:
    ...
    def __init__(self, opt):
        ...
        if self.isTrain:
            ...
            self.gpt_mask = self.get_mask(20)
    ...
    def forward(self):
        ...
        if self.isTrain:
            self.harmonized = self.netG(self.inputs, deformable_att=self.deformable_att, gpt_mask=self.gpt_mask)
        else:
            self.harmonized = self.netG(self.test_inputs, deformable_att=self.deformable_att, gpt_mask=self.gpt_mask, is_test=True)
            # flops, params = profile(self.netG, (self.inputs,self.deformable_att,self.gpt_mask,True,))
            # print('flops: %.2f GFlops, params: %.2f M' % (flops / pow(10, 9), params / pow(10, 6)))
