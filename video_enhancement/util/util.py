...
def tensor2im(input_image, imtype=np.uint8):
    ...
    if not isinstance(input_image, np.ndarray):
        ...
        if image_numpy.shape[0] == 1:  # grayscale to RGB
            image_numpy = np.tile(image_numpy, (3, 1, 1))*2.0-1.0
...
def window_partition(x, window_size):
    ...
    windows = x.permute(0, 2, 4, 1, 3, 5).flatten(1,2).flatten(0,1).flatten(1,2)
    return windows
...
def window_reverse(windows, window_size, H, W):
    ...
    Returns:
        x: (B, H, W, C)
    """
    # [1280, 64, 256] -> [20, 4096, 256]
    # B = int(windows.shape[0] / (H * W / window_size / window_size))
    B = int(windows.shape[0] / (window_size * window_size))
    x = windows.view(B, H // window_size, W // window_size, window_size, window_size, -1)
    # x = x.permute(0, 1, 3, 2, 4, 5).contiguous().view(B, H, W, -1)
    x = x.permute(0, 1, 3, 2, 4, 5).flatten(1,2).flatten(2,3).flatten(1,2)
    return x

def padding_mask_partitaion(x, window_size):
    B,L = x.shape
    x = x.view(int(math.sqrt(B)),int(math.sqrt(B)),L)
    H,W,L = x.shape
    x = x.view(H//window_size, window_size, W//window_size, window_size, L)
    windows = x.permute(0,2,1,3,4).flatten(0,1).flatten(1)
    return windows
