...
class TransformerEncoderLayer(nn.Module):
    ...
    def forward_pre(self, src,
        ...
        src2 = self.self_attn(q, k, value=src2, attn_mask=src_mask,
                              key_padding_mask=src_key_padding_mask)[0]  # [token_num, bs, dim]
        src = src + self.dropout1(src2)
        src2 = self.norm2(src)
...
class TransformerDecoderLayer(nn.Module):
    ...
    def forward_post(self, tgt, memory,
        ...
        if len(tgt.size()) == 4:
            tgt_token_num = tgt.size(2)
            tgt = tgt.transpose(2,1).flatten(0,1)
        ...
        if len(memory.size()) == 4:
            bs, hw, token_num, dim = memory.size()
            tgt = tgt.view(bs, tgt_token_num, hw, dim).transpose(2,1).flatten(0,1)
            ...
            tgt = tgt.view(bs, hw, tgt_token_num, dim)
            tgt2 = tgt2.view(bs, hw, tgt_token_num, dim)
            # tgt2 = None
            # for i in range(bs):
            #     t, m = tgt[i], memory[i]
            #     tgt2_temp = self.multihead_attn(query=self.with_pos_embed(t, query_pos),
            #                        key=self.with_pos_embed(m, pos),
            #                        value=m, attn_mask=memory_mask,
            #                        key_padding_mask=memory_key_padding_mask)[0]
            #     if tgt2 is None:
            #         tgt2 = tgt2_temp.unsqueeze(0)
            #     else:
            #         tgt2 = torch.cat([tgt2, tgt2_temp.unsqueeze(0)], dim=0)
        else:
            tgt2 = self.multihead_attn(query=self.with_pos_embed(tgt, query_pos),
                                    key=self.with_pos_embed(memory, pos),
                                    value=memory, attn_mask=memory_mask,
